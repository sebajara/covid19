...
sizes = 2
topn = 20
maxx = np.max(world_df['dateRep'].unique()-min_date)/np.timedelta64(1, 'D')-38

def get_days(sdf, min_date=min_date):
...
def country_legend(axes, country, hlcountries, legendys, yval, colors=colors, maxx=maxx):
...
for (i, country) in enumerate(countries):
    ...
    (days, deltadays) = get_days(sdf)
    color = country_legend(axes, country, hlcountries, legendys, yval, colors=colors, maxx=maxx)
